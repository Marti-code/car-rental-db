...
CREATE OR REPLACE ALGORITHM = UNDEFINED DEFINER = `huza_martyna`@`%` SQL SECURITY DEFINER VIEW `projekt_dylag_herczyk_huza`.`v_ActiveReservations` AS SELECT
  `res`.`ReservationID` AS `ReservationID`,
  `res`.`ReservationDate` AS `ReservationDate`,
  `res`.`PickupDate` AS `PickupDate`,
  `res`.`ReturnDate` AS `ReturnDate`,
  ...
  `m`.`MakeName` AS `Make`,
  `car`.`Model` AS `Model`,
  `car`.`LicensePlateNumber` AS `LicensePlateNumber` 
FROM
  ((((
          `reservation` `res`
          JOIN `customer` `c` ON ( `res`.`CustomerID` = `c`.`CustomerID` ))
        JOIN `reservationcar` `rc` ON ( `res`.`ReservationID` = `rc`.`ReservationID` ))
      JOIN `car` ON ( `rc`.`CarID` = `car`.`CarID` ))
  LEFT JOIN `make` `m` ON ( `car`.`MakeID` = `m`.`MakeID` )) 
WHERE
  `res`.`ReservationStatus` = 'Active' 
  AND `res`.`PickupDate` >= curdate() 
ORDER BY
  `res`.`PickupDate`;
...
CREATE OR REPLACE ALGORITHM = UNDEFINED DEFINER = `huza_martyna`@`%` SQL SECURITY DEFINER VIEW `projekt_dylag_herczyk_huza`.`v_AvailableCars` AS SELECT
  `c`.`CarID` AS `CarID`,
  `c`.`LicensePlateNumber` AS `LicensePlateNumber`,
  `m`.`MakeName` AS `Make`,
  `c`.`Model` AS `Model`,
  `c`.`Year` AS `Year`,
  `c`.`Color` AS `Color`,
  ...
  `t`.`TransmissionTypeName` AS `TransmissionType`,
  group_concat( DISTINCT `f`.`FeatureName` SEPARATOR ', ' ) AS `Features`,
  group_concat( DISTINCT `a`.`AmenityName` SEPARATOR ', ' ) AS `Amenities` 
FROM
  ((((((((
                  `car` `c`
                  JOIN `vehicletype` `vt` ON ( `c`.`VehicleTypeID` = `vt`.`VehicleTypeID` ))
                JOIN `vehiclecategory` `vc` ON ( `vc`.`CategoryID` = `vt`.`CategoryID` ))
              JOIN `transmissiontype` `t` ON ( `t`.`TransmissionTypeID` = `vt`.`TransmissionTypeID` ))
            LEFT JOIN `make` `m` ON ( `c`.`MakeID` = `m`.`MakeID` ))
          LEFT JOIN `carfeature` `cf` ON ( `c`.`CarID` = `cf`.`CarID` ))
        LEFT JOIN `feature` `f` ON ( `cf`.`FeatureID` = `f`.`FeatureID` ))
      LEFT JOIN `caramenity` `ca` ON ( `c`.`CarID` = `ca`.`CarID` ))
  LEFT JOIN `amenity` `a` ON ( `ca`.`AmenityID` = `a`.`AmenityID` )) 
WHERE
...
GROUP BY
  ...
  `c`.`DailyRate`,
  `vc`.`CategoryName`,
  `t`.`TransmissionTypeName`;

-- ----------------------------
-- View structure for v_CarsCountRentedByMonth
-- ----------------------------
DROP VIEW IF EXISTS `v_CarsCountRentedByMonth`;
CREATE OR REPLACE ALGORITHM = UNDEFINED DEFINER = `huza_martyna`@`%` SQL SECURITY DEFINER VIEW `projekt_dylag_herczyk_huza`.`v_CarsCountRentedByMonth` AS SELECT YEAR
  ( `r`.`RentalDate` ) AS `RentalYear`,
  MONTH ( `r`.`RentalDate` ) AS `RentalMonth`,
  count( DISTINCT `rc`.`CarID` ) AS `CarsRented` 
FROM
  (
    `rental` `r`
...
GROUP BY
  ...
  MONTH ( `r`.`RentalDate` ) 
ORDER BY
  YEAR ( `r`.`RentalDate` ) DESC,
  MONTH ( `r`.`RentalDate` ) DESC;

-- ----------------------------
-- View structure for v_CarUtilityStats
-- ----------------------------
DROP VIEW IF EXISTS `v_CarUtilityStats`;
CREATE OR REPLACE ALGORITHM = UNDEFINED DEFINER = `huza_martyna`@`%` SQL SECURITY DEFINER VIEW `projekt_dylag_herczyk_huza`.`v_CarUtilityStats` AS SELECT
  `car`.`CarID` AS `CarID`,
  `car`.`LicensePlateNumber` AS `LicensePlateNumber`,
  `m`.`MakeName` AS `Make`,
  `car`.`Model` AS `Model`,
  sum( `rc`.`RentalDuration` ) AS `TotalRentalDays`,
  sum( `rc`.`DailyRateApplied` * `rc`.`RentalDuration` ) AS `TotalRevenue` 
FROM
  ((
      `car`
      LEFT JOIN `make` `m` ON ( `car`.`MakeID` = `m`.`MakeID` ))
  LEFT JOIN `rentalcar` `rc` ON ( `car`.`CarID` = `rc`.`CarID` )) 
GROUP BY
  `car`.`CarID`,
...
CREATE OR REPLACE ALGORITHM = UNDEFINED DEFINER = `huza_martyna`@`%` SQL SECURITY DEFINER VIEW `projekt_dylag_herczyk_huza`.`v_EmployeeCountByRoles` AS SELECT
...
GROUP BY
  `r`.`RoleName` 
ORDER BY
  count( DISTINCT `e`.`EmployeeID` ) DESC;

-- ----------------------------
-- View structure for v_EmployeeInfoWithAccountStatus
-- ----------------------------
DROP VIEW IF EXISTS `v_EmployeeInfoWithAccountStatus`;
CREATE OR REPLACE ALGORITHM = UNDEFINED DEFINER = `huza_martyna`@`%` SQL SECURITY DEFINER VIEW `projekt_dylag_herczyk_huza`.`v_EmployeeInfoWithAccountStatus` AS SELECT
  ...
  `r`.`RoleName` AS `RoleName`,
  `e`.`AccountStatus` AS `AccountStatus` 
FROM
  ((
      `employee` `e`
      JOIN `employeerole` `er` ON ( `e`.`EmployeeID` = `er`.`EmployeeID` ))
...
CREATE OR REPLACE ALGORITHM = UNDEFINED DEFINER = `huza_martyna`@`%` SQL SECURITY DEFINER VIEW `projekt_dylag_herczyk_huza`.`v_InvoiceSummaryByCustomer` AS SELECT
  `invoice`.`CustomerID` AS `CustomerID`,
  count( `invoice`.`InvoiceID` ) AS `TotalInvoices`,
  sum( CASE WHEN `invoice`.`InvoiceStatus` = 'Paid' THEN `invoice`.`TotalGrossAmount` ELSE 0 END ) AS `TotalPaid`,
  sum( CASE WHEN `invoice`.`InvoiceStatus` = 'Unpaid' THEN `invoice`.`TotalGrossAmount` ELSE 0 END ) AS `TotalUnpaid`,
  sum( CASE WHEN `invoice`.`DueDate` < curdate() AND `invoice`.`InvoiceStatus` = 'Unpaid' THEN `invoice`.`TotalGrossAmount` ELSE 0 END ) AS `OverdueAmount` 
FROM
  `invoice` 
GROUP BY
  `invoice`.`CustomerID`;
...
CREATE OR REPLACE ALGORITHM = UNDEFINED DEFINER = `huza_martyna`@`%` SQL SECURITY DEFINER VIEW `projekt_dylag_herczyk_huza`.`v_PopularCars` AS SELECT
  `c`.`CarID` AS `CarID`,
  `c`.`LicensePlateNumber` AS `LicensePlateNumber`,
  `m`.`MakeName` AS `Make`,
  `c`.`Model` AS `Model`,
  count( `rc`.`RentalID` ) AS `TimesRented` 
FROM
  ((
      `car` `c`
      LEFT JOIN `make` `m` ON ( `c`.`MakeID` = `m`.`MakeID` ))
  LEFT JOIN `rentalcar` `rc` ON ( `c`.`CarID` = `rc`.`CarID` )) 
GROUP BY
  `c`.`CarID`,
  `c`.`LicensePlateNumber`,
  `m`.`MakeName`,
...
SELECT YEAR
  ( `r`.`RentalDate` ) AS `Year`,
  MONTH ( `r`.`RentalDate` ) AS `Month`,
  sum( `r`.`TotalGrossAmount` ) AS `TotalRevenue` 
FROM
  `rental` `r` 
GROUP BY
  YEAR ( `r`.`RentalDate` ),
...
FROM
  ...
  LEFT JOIN `make` `m` ON ( `car`.`MakeID` = `m`.`MakeID` )) 
WHERE
  `res`.`PickupDate` BETWEEN curdate() 
...
-- ----------------------------
DROP PROCEDURE IF EXISTS `ExtendRentalTime`;
delimiter ;;
...
BEGIN
  ...
  WHERE CustomerID = input_CustomerID
    ...
    AND InvoiceID = LAST_INSERT_ID(); # This will work. Why? Cuz it is for a given session (no need to worry about the other users) and since the last insert was into an invoice table (the insert above) we will get the last invoice ID
...
delimiter ;;
CREATE PROCEDURE `UpdateRentalStatus`(IN vRentalID INT)
BEGIN
    DECLARE vCarID INT;
    ...
    SELECT CarID INTO vCarID
    FROM rentalcar
    WHERE RentalID = vRentalID;
    ...
    SELECT ReservationID INTO vReservationID
    FROM rental
    WHERE RentalID = vRentalID;
